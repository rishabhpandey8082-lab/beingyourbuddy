...
const WaveformVisualizer = forwardRef<HTMLDivElement, WaveformVisualizerProps>(
  ({ isActive, variant = "bars", color = "primary", className = "" }, ref) => {
    const colorClasses = {
      primary: "bg-primary",
    ...
    if (variant === "bars") {
      return (
        <div ref={ref} className={`flex items-center justify-center gap-1 h-8 ${className}`}>
          {[...Array(5)].map((_, i) => (
            <motion.div
              key={i}
    ...
    // Wave variant
    return (
      <div ref={ref} className="flex items-end justify-center gap-0.5 h-8">
        {[...Array(12)].map((_, i) => (
          <motion.div
            key={i}
            className={`w-1 rounded-full ${colorClasses[color]}`}
